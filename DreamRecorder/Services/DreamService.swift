    ...
    func interpretDream(dream: Dream, reflection: Reflection?, teller: FortuneTeller, userId: String) async throws {
        do {
            ...
            if let reflection = reflection {
                promptText = """
                以下の「昨日の日記」と「夢の内容」を分析し、あなたの設定されたキャラクター性（口調や態度）を崩さずに占ってください。
                回答は必ず挨拶と以下の3つのセクションを含み、字数制限を守って記述してください。
                挨拶には，占いにおとづれてくれたユーザーへの歓迎の言葉を必ず含めてください。
                各セクションの冒頭に【 】で囲った見出しをつけてください。

                【ひとこと】（30文字以内）
                占い結果を象徴する、短く印象的な一言（キャッチコピーのように）。

                【夢占い】（150文字程度）
                昨日の日記はいったん切り離し、夢に出てきたシンボルや状況が持つ本来の意味、深層心理からのメッセージを解説してください。

                【日記との繋がり】（150文字程度）
                昨日の日記に書かれた出来事や感情が、この夢にどう影響しているか（因果関係）、あるいは夢が日記の内容に対してどのような示唆を与えているかを分析してください。
                ...
                promptText = """
                以下の「夢の内容」から深層心理を分析し、あなたの設定されたキャラクター性（口調や態度）を崩さずに占ってください。
                回答は必ず挨拶と以下の2つのセクションに分け、字数制限を守って記述してください。
                挨拶には，占いにおとづれてくれたユーザーへの歓迎の言葉を必ず含めてください。
                各セクションの冒頭に【 】で囲った見出しをつけてください。
                ...
                【夢占い】（150文字程度）
                夢に出てきたシンボルや状況が持つ意味、深層心理からのメッセージを詳しく解説してください。
